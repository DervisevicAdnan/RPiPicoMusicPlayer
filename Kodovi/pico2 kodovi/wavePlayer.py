...
class wavePlayer:
    ...
    def continueSong(self):
        self.resumeFlag = True
        if self.pauseFlag and self.pausedFileName != "":
            self.play(self.pausedFileName)
    ...
    def play(self,filename):
        ...
        if self.resumeFlag:
            frameLeft = self.pausedFrameLeft
            f.setpos(frameCount-self.pausedFrameLeft)
            self.resumeFlag = False

        while not self.stopFlag and frameLeft>0:
            if self.pauseFlag:
                self.pausedFrameLeft = frameLeft
                self.pausedFileName = filename
                self.stop()
